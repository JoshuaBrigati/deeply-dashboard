...
export function InsightCard({
  ...
  useEffect(() => {
    if (dimension && !insight && !isLoading) {
      fetchInsight();
    }
  }, [dimension, insight, isLoading, fetchInsight]);
              ...
              return (
                <div key={sub.name} className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    <span className="font-medium text-user w-5 text-right">
                      {sub.userScore}
                    </span>
                    <span className="text-muted-foreground">/</span>
                    <span className="font-medium text-partner w-5">
                      {sub.partnerScore}
                    </span>
                    {subGap >= 10 && (
